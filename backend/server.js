...
if (process.env.EMERGENCY_PURGE !== "0") {
  try {
    ...
    const DATA_DIR_BOOT =
      process.env.DATA_DIR || pathBoot.resolve(__dirname, "../data/servers");
    ...
    if (freeMb < 200) {
      console.log(
        `🚨 EMERGENCY PURGE: only ${freeMb}MB free on ${VOLUME_ROOT_BOOT} — wiping logs/cache/crash-reports across all servers`,
      );
      let freed = 0,
        hits = 0;
      ...
      function dirSizeSync(p) {
        ...
        try {
          ...
          if (st.isFile()) t = st.size;
            ...
            for (const c of fsSync.readdirSync(p))
              t += dirSizeSync(pathBoot.join(p, c));
          }
        } catch {}
        return t;
      }
      try {
        ...
        for (const e of dirs) {
          ...
          for (const d of PURGE) {
            ...
            if (sz > 0) {
              try {
                fsSync.rmSync(sub, { recursive: true, force: true });
                freed += sz;
                hits++;
              } catch {}
            }
        ...
        console.warn("[emergency-purge] readdir failed:", err.message);
      }
        ...
        try {
          for (const f of fsSync.readdirSync(tmpDir)) {
            ...
            try {
              ...
              if (st.isFile() && Date.now() - st.mtimeMs > 5 * 60_000) {
                freed += st.size;
      ...
      console.log(
        `🚨 EMERGENCY PURGE freed ${Math.round(freed / 1024 / 1024)}MB across ${hits} items — boot continuing`,
      );
    }
  } catch (err) {
...
app.use("/api/auth/forgot", authLimit);
app.use("/api/auth/reset", authLimit);
...
app.get("/api/health", async (req, res) => {
  ...
  try {
    ...
    if (fs.existsSync(CACHE_DIR)) {
      ...
      // The verified-versions state — proves which LATEST builds passed
      // the boot test and are safe to serve to users.
      try {
        const state = JSON.parse(
          fs.readFileSync(path.join(CACHE_DIR, "_versions.json"), "utf8"),
        );
    ...
    try {
      ...
      /* module load may fail in non-jvm mode */
    }
  } catch (err) {
  ...
  res.json({
    ...
    public_mc_host:
      process.env.PUBLIC_MC_HOST || process.env.RAILWAY_PUBLIC_DOMAIN || null,
  });
});
...
app.get("/api/status", async (req, res) => {
  try {
    ...
    const running = db
      .prepare(
        `SELECT COUNT(*) AS c FROM servers WHERE status IN ('online','running','starting')`,
      )
      .get().c;
    ...
    const onlineServers = db
      .prepare(
        `SELECT s.*, p.ram_mb FROM servers s JOIN plans p ON s.plan_id = p.id WHERE s.status IN ('online','running') AND s.container_id LIKE 'jvm-%' LIMIT 20`,
      )
      .all();
    for (const s of onlineServers) {
      try {
        const st = await dc.getStats(s);
        totalPlayers += st?.players || 0;
      } catch {}
    }
    res.json({
      ok: true,
...
server.on("upgrade", (req, socket, head) => {
  ...
  if (!pathname.startsWith("/ws/servers/")) {
    socket.destroy();
    return;
  }
...
server.listen(PORT, async () => {
  ...
  if (backend === "jvm")
    console.log(
      `✓ JVM available; public MC port: ${process.env.MC_PORT || 25565}`,
    );
  ...
  try {
    ...
    if (!anyAdmin) {
      ...
      if (u1) {
        ...
        console.log(
          `👑 Auto-promoted user "${u1.username}" (id=1) to admin — no admin existed.`,
        );
      }
    }
  } catch (err) {
  ...
  try {
    ...
    const DATA_DIR =
      process.env.DATA_DIR || pathMod.resolve(__dirname, "../data/servers");
    ...
    for (const e of entries) {
      ...
      if (size > 100 * 1024 * 1024 && !aggressive) {
        console.warn(
          `🧹 Janitor: skipping orphan ${e.name} (${Math.round(size / 1024 / 1024)}MB > 100MB threshold; set JANITOR_AGGRESSIVE=1 to delete)`,
        );
        skippedBig++;
        continue;
      }
    ...
    console.log(
      `🧹 Janitor: ${kept} kept · ${removed} removed (${Math.round(freed / 1024 / 1024)}MB freed)${skippedBig ? " · " + skippedBig + " skipped (size>100MB)" : ""}`,
    );
  } catch (err) {
      ...
      try {
        ...
        for (const s of rows) {
          ...
          if (idleMs > idleMin * 60_000) {
            console.log(
              `💤 Idle-stop: ${s.name} (${s.id}) — 0 players for ${Math.round(idleMs / 60000)}min · saving + stopping`,
            );
            try {
              // 1) Explicit save: world chunks, level.dat, player files, stats/, advancements/
              const cid =
                String(s.container_id || "").replace(/^jvm-/, "") || s.id;
              const state = jvm.__getState ? jvm.__getState(cid) : null;
              if (state?.proc?.stdin && !state.proc.killed) {
                try {
                  state.proc.stdin.write("save-all flush\n");
                } catch {}
                try {
                  state.proc.stdin.write("save-off\n");
                } catch {}
        ...
        console.warn("[idle-stop] failed:", err.message);
      }
    }, 60_000);
    console.log(
    ...
    setInterval(async () => {
      try {
        ...
        for (const s of rows) {
          ...
          const onSafe =
            String(s.type).toLowerCase() === SAFE_TYPE &&
            s.version === SAFE_VERSION;
          const cooledHeal =
            !s.auto_healed_at || now - s.auto_healed_at >= HEAL_COOLDOWN_MS;
          ...
          if (stats?.oom && !repeatedOom && oomRetryCooldown) {
            console.log(
              `🔁 OOM retry: ${s.name} (${s.id}) — same version (${s.type} ${s.version}), attempt ${(s.auto_restart_count || 0) + 1}/2`,
            );
            try {
          ...
          if (isFree && !onSafe && cooledHeal && stats?.oom && repeatedOom) {
            console.log(
              `🔧 Auto-heal: ${s.name} (${s.id}) — ${s.type} ${s.version} OOM'd → swapping to ${SAFE_TYPE} ${SAFE_VERSION}`,
            );
            try {
              const fsp = require("fs/promises");
              const pathMod = require("path");
              ...
              await fsp
                .unlink(pathMod.join(serverRoot, "server.jar"))
                .catch(() => {});
              ...
              for (const f of STALE_CONFIGS) {
                try {
                  await fsp.rm(pathMod.join(serverRoot, f), {
                    recursive: true,
                    force: true,
                  });
                } catch {}
              ...
              for (const d of ["crash-reports", "cache", "logs"]) {
                try {
                  await fsp.rm(pathMod.join(serverRoot, d), {
                    recursive: true,
                    force: true,
                  });
                } catch {}
              ...
              db.prepare(
                `UPDATE servers SET type = ?, version = ?, status = ?, auto_healed_at = ? WHERE id = ?`,
          ...
          try {
            ...
            if (!fixed.ok) {
              console.warn(`[auto-fix] ${s.id} failed:`, fixed.error);
              continue;
            }
            console.log(
              `🔧 Auto-fix: ${s.name} (${s.id}) — ${fixed.kind} (${fixed.diagnosis})`,
              fixed.detail || "",
            );
        ...
        console.warn("[auto-heal] loop failed:", err.message);
      }
    }, 20_000);
    console.log(
      `🔧 Auto-heal + auto-fix loop enabled (OOM → safe JAR; port/JAR/world/plugin → targeted fix + restart)`,
    );
  }
  ...
  if (backend === "jvm" && process.env.AUTO_RESTART !== "0") {
    ...
    const WINDOW_MS =
      parseInt(process.env.AUTO_RESTART_WINDOW || "15", 10) * 60 * 1000;
    ...
    setInterval(async () => {
      try {
        for (const [id, info] of dc.getCrashes()) {
          ...
          console.log(
            `🔁 Auto-restart: ${s.name} (${id}) — crash code=${info.code} signal=${info.signal || "none"}, attempt ${count + 1}/${MAX}`,
          );
          try {
            const r = await dc.startServer(s);
            db.prepare(
        ...
        console.warn("[auto-restart] loop failed:", err.message);
      }
    }, 15_000);
    console.log(
      `🔁 Auto-restart loop enabled (max ${MAX} per ${WINDOW_MS / 60000}min)`,
    );
  }
  ...
  if (process.env.DISK_JANITOR !== "0") {
    ...
    async function dirSize(p) {
      let t = 0;
      try {
        const st = await fspMod.stat(p);
        if (st.isFile()) t = st.size;
        else if (st.isDirectory()) {
          const ee = await fspMod.readdir(p);
          for (const c of ee) t += await dirSize(pathMod.join(p, c));
        }
      } catch {}
      return t;
    }
      ...
      try {
        ...
        for (const id of ids) {
          ...
          for (const d of PURGE_DIRS) {
            ...
            if (sz > 0) {
              try {
                await fspMod.rm(sub, { recursive: true, force: true });
                freed += sz;
                hits++;
              } catch {}
            }
          ...
          try {
            ...
            for (const e of top) {
              ...
              if (PURGE_FILE_RE.some((re) => re.test(e.name))) {
                ...
                try {
                  const st = await fspMod.stat(full);
                  freed += st.size;
                  await fspMod.unlink(full);
                  hits++;
                } catch {}
              }
            }
        ...
        for (const tmpDir of ["/tmp", pathMod.join(VOLUME_ROOT, "tmp")]) {
          try {
            ...
            for (const f of files) {
              ...
              try {
                const st = await fspMod.stat(full);
                if (st.isFile() && Date.now() - st.mtimeMs > 60 * 60 * 1000) {
                  freed += st.size;
                  await fspMod.unlink(full);
                  hits++;
                }
        ...
        if (freed > 0)
          console.log(
            `🧹 Janitor: freed ${Math.round(freed / 1024 / 1024)}MB across ${hits} items`,
          );
      } catch (err) {
        console.warn("[janitor] loop failed:", err.message);
      }
    }, intMin * 60_000);
    console.log(
      `🧹 Disk janitor loop enabled (${intMin}min interval — clears logs/cache/crash-reports + old /tmp uploads)`,
    );
  }
    ...
    setInterval(
      async () => {
        ...
        `,
          ...
          if (freed > 0)
            console.log(
              `🗑  Backup prune: freed ${Math.round(freed / 1024 / 1024)}MB across ${hits} backup files (${pruneDays}d+ idle)`,
            );
        } catch (err) {
          console.warn("[backup-prune] loop failed:", err.message);
        }
      },
      6 * 60 * 60_000,
    ); // every 6h
    console.log(
      `🗑  Backup prune loop enabled (every 6h, deletes backups for servers offline ${pruneDays}+ days)`,
    );
  }
  ...
  if (backend === "jvm" && process.env.SCHEDULED_RESTART !== "0") {
    ...
    setInterval(async () => {
      try {
        ...
        for (const s of due) {
          console.log(
            `⏰ Scheduled restart: ${s.name} (${s.id}) at ${slot} UTC`,
          );
          try {
            ...
            if (state?.proc?.stdin && !state.proc.killed) {
              try {
                state.proc.stdin.write(
                  "say §e[CraftHost] Scheduled restart in 5s — saving world…\n",
                );
              } catch {}
              try {
                state.proc.stdin.write("save-all flush\n");
            ...
            db.prepare(
              "UPDATE servers SET status = ?, last_scheduled_restart_at = ? WHERE id = ?",
        ...
        console.warn("[scheduled-restart] loop failed:", err.message);
      }
    }, 60_000);
          ...
          for (const { id, name, user_id } of stale) {
            ...
            try {
              ...
              async function sz(p) {
                let t = 0;
                try {
                  const s = await fspMod.stat(p);
                  if (s.isFile()) t = s.size;
                  else if (s.isDirectory()) {
                    const ee = await fspMod.readdir(p);
                    for (const c of ee) t += await sz(pathMod.join(p, c));
                  }
                } catch {}
                return t;
              }
              ...
              try {
                await dc.removeServer({ id, container_id: `jvm-${id}` });
              } catch {}
              db.prepare("DELETE FROM servers WHERE id = ?").run(id);
              freed += before;
              removed++;
              console.log(
                `🗑  Auto-delete: ${name} (${id}) — inactive ${dDays}d+ — freed ${Math.round(before / 1024 / 1024)}MB`,
              );
            } catch (err) {
              console.warn(`[auto-delete] ${id} failed:`, err.message);
            }
          }
          if (removed > 0)
            console.log(
              `🗑  Auto-delete swept ${removed} server(s), freed ${Math.round(freed / 1024 / 1024)}MB`,
            );
        } catch (err) {
          console.warn("[auto-delete] loop failed:", err.message);
        }
      },
      6 * 60 * 60_000,
    ); // every 6h
    console.log(
      `🗑  Auto-delete loop enabled (every 6h, deletes servers offline ${dDays}+ days)`,
    );
  }
  ...
  if (backend === "jvm" && process.env.AUTO_BACKUP !== "0") {
    ...
    setInterval(async () => {
      try {
        ...
        for (const s of rows) {
          try {
            try {
              await dc.sendRcon(s, "/save-all");
              await dc.sendRcon(s, "/save-off");
            } catch {}
            await new Promise((r) => setTimeout(r, 1500));
            const info = await bk.createBackup(s.id, {
              retention: s.backups_count || 1,
            });
            try {
              await dc.sendRcon(s, "/save-on");
            } catch {}
            console.log(
              `  ✓ ${s.name}: ${info.filename} (${(info.size / 1024 / 1024).toFixed(1)} MB)`,
            );
          } catch (err) {
    ...
    console.log(
      `⏰ Auto-backup scheduler enabled (every ${intervalHours}h, paid plans only)`,
    );
  }
  ...
  if (
    backend === "jvm" &&
    process.env.AUTO_SMOKE !== "0" &&
    jvmCtl.checkJarCacheHealth
  ) {
    const runHealth = async () => {
      try {
        const r = await jvmCtl.checkJarCacheHealth();
        const newVer = (r.new_versions || []).length
          ? `  🆕 new: ${r.new_versions.join(", ")}`
          : "";
        if (r.real_failed > 0) {
          console.error(
            `🚨 JAR health: ${r.real_failed} NEW version(s) FAILED real-test — rolled back${newVer}`,
          );
        } else if (r.real_tested > 0) {
          console.log(
            `✅ JAR health: ${r.real_tested} new version(s) verified real-runnable${newVer}`,
          );
        } else if (r.repaired > 0 || r.errors > 0) {
          console.log(
            `🚨 JAR health: ${r.healthy}✓ ${r.repaired} repaired · ${r.errors} errors (of ${r.scanned})`,
          );
        } else {
    ...
    console.log(
      "🩺 JAR health probe scheduled (every 10 min, auto-repair enabled)",
    );
  }
  ...
  try {
    db.prepare(
      "UPDATE servers SET tunnel_host = NULL, tunnel_port = NULL",
    ).run();
  } catch {}
  ...
  try {
    ...
    if (orphans.length) {
      ...
      console.log(
        `🧹 Cleaned ${orphans.length} test-user orphans (${srvs} servers, ${dirs} data dirs)`,
      );
    }
  } catch (err) {
  ...
  if (backend === "jvm" && process.env.JVM_AUTO_RESUME !== "0") {
    try {
      ...
      if (skipped > 0) {
        console.log(
          `↻ Auto-resume: keeping ${rows.length} most-recent server(s), skipping ${skipped} older one(s) (raise MAX_AUTO_RESUME to change).`,
        );

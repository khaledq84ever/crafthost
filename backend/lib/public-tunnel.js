...
function isAvailable() { return bore.isAvailable() || playit.isAvailable(); }
    ...
    if (secret) {
      try {
        ...
        if (addr && addr.host && addr.port) {
          ...
          } catch (e) { console.warn('[pubtun] db update:', e.message); }
          return addr;
        }
        console.warn('[pubtun] playit-java returned no address — falling back to bore');
      } catch (e) {
        console.warn('[pubtun] playit-java failed, falling back to bore:', e.message);
      }
    }
  }
...
function stop(serverId) {
  try { bore.stop(serverId); } catch {}
  try { playit.stopJava(serverId); } catch {}
  try {

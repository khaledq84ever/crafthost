...
// Per-server Geyser UDP port — mirrors jvm-controller.geyserUdpPort. Passed to
// playit.start for Bedrock; ignored in the legacy shared-tunnel mode.
function bedrockLocalPort(server) {
  const offset = Math.max(0, parseInt(server.port, 10) - 25565);
...
function nextSlotFor(userId) {
  const row = db
    .prepare(
      "SELECT COALESCE(MAX(user_slot), 0) AS m FROM servers WHERE user_id = ?",
    )
    .get(userId);
  return (row?.m || 0) + 1;
}
...
const MAX_SERVERS_PER_USER = parseInt(
  process.env.MAX_SERVERS_PER_USER || "3",
  10,
);
...
function isUnlimitedUser(user) {
  ...
  return (
    UNLIMITED_USERS.includes(uname) ||
...
function setSharedPlayitSecret(secret) {
  if (!secret) return;
  db.prepare(
    `INSERT INTO app_settings (key, value) VALUES ('playit_shared_secret', ?)
...
async function createServerForUser(user, opts, ip) {
  ...
  if (!isUnlimitedUser(user)) {
    const existing = db
      .prepare("SELECT COUNT(*) AS c FROM servers WHERE user_id = ?")
      .get(user.id);
    if (existing.c >= MAX_SERVERS_PER_USER) {
      const e = new Error(
        `Server limit reached — you can create up to ${MAX_SERVERS_PER_USER} servers. Delete one to make room.`,
      );
      e.status = 409;
      throw e;
    }
  ...
  try {
    ...
    const e = new Error("No free server ports");
    e.status = 503;
    throw e;
  }
  ...
  const anyPublic = db
    .prepare("SELECT 1 FROM servers WHERE is_public = 1 LIMIT 1")
    .get();
  ...
  const enriched = {
    ...server,
    ram_mb: planRow.ram_mb,
    cpu_cores: planRow.cpu_cores,
  };

  try {
    const r = await dc.createServer(enriched);
    db.prepare(
      "UPDATE servers SET container_id = ?, status = ? WHERE id = ?",
    ).run(r.containerId, "created", id);
    ...
    try {
      ...
      const DATA_DIR =
        ...
        pathMod.resolve(__dirname, "../../data/servers");
    ...
    if (seed && typeof seed === "string" && seed.trim()) {
      try {
        ...
        let existing = "";
        try {
          existing = await fsp.readFile(propsPath, "utf8");
        } catch {}
    ...
    if (Array.isArray(seed_plugins) && seed_plugins.length) {
      seedDefaultPlugins(user.id, id, seed_plugins, version).catch((err) =>
        console.warn("[seed]", err.message),
      );
    } else if (seed_plugins === true) {
      seedDefaultPlugins(user.id, id, ["Vebnzrzj"], version).catch((err) =>
        console.warn("[seed]", err.message),
      );
    }
...
router.post("/clone", async (req, res) => {
  ...
  try {
    if (!skipWorld) {
      ...
      if (fs.existsSync(srcDir))
        await copyDir(srcDir, path.join(dstRoot, "plugins"));
    }
  ...
  try {
    ...
    if (quotaErr) {
      ...
      audit(req.user.id, "server.start", out.id, req.ip, {
        auto: true,
        cloned_from: src.id,
      });
      autoStarted = true;
      ...
      if (pubtun.isAvailable()) {
        try {
          const t = await pubtun.start(
            out.id,
            internalListenPort({ port: out.port }),
          );
          if (t) out.proxy = { host: t.host, port: t.port };
        } catch (err) {
          console.warn("[clone] tunnel start failed:", err.message);
        }
      }
    }
  } catch (err) {
  ...
  audit(req.user.id, "server.clone", out.id, req.ip, {
    source_id: src.id,
    copy: copyStats,
  });
...
router.get("/health-check", async (req, res) => {
  ...
  for (const s of rows) {
    ...
    const healedRecently =
      s.auto_healed_at && now - s.auto_healed_at * 1000 < HEALED_WINDOW_MS;
    const restartedRecently =
      s.last_auto_restart_at &&
      now - s.last_auto_restart_at * 1000 < RESTART_WINDOW_MS;
    ...
    if (stats?.oom || state?.oom) {
      if (healedRecently) {
        ...
        issues.push({
          ...
          code: "oom",
          message:
            "Server ran out of memory (OOM kill). Try a lighter JAR or upgrade plan.",
        });
      }
    }
    ...
    if (stats?.online && stats.cpu >= 95) {
      issues.push({
        severity: "warn",
        code: "cpu_high",
        message: `CPU pegged at ${stats.cpu}% — may cause TPS drops.`,
      });
    }
    if (
      stats?.online &&
      stats.ram_max &&
      stats.ram_used / stats.ram_max >= 0.92
    ) {
      issues.push({
        severity: "warn",
        code: "ram_high",
    ...
    if (restartedRecently && (s.auto_restart_count || 0) >= 3) {
      issues.push({
        severity: "warn",
        code: "restart_loop",
        message: `Auto-restarted ${s.auto_restart_count}× recently. Server may be unstable.`,
      });
    }
    if (
      s.status === "starting" &&
      state?.startedAt &&
      now - state.startedAt > 120000
    ) {
      issues.push({
        severity: "warn",
        code: "slow_boot",
        message: 'Server has been "starting" for over 2 minutes.',
      });
    }
    if (healedRecently) {
      issues.push({
        severity: "info",
        code: "auto_healed",
        message: "✓ Auto-healed recently (rolled to safe JAR after OOM).",
      });
    }
    if (restartedRecently) {
      issues.push({
        severity: "info",
        code: "auto_restarted",
        message: `✓ Auto-restarted on crash (${s.auto_restart_count}×).`,
      });
    }
    const FIX_WINDOW_MS = 30 * 60 * 1000; // show fix pill for 30 min
    if (
      s.last_auto_fix_at &&
      now - s.last_auto_fix_at * 1 < FIX_WINDOW_MS &&
      s.last_auto_fix_kind
    ) {
      const KIND_LABEL = {
        port: "port collision",
        jar: "corrupt JAR",
        world: "corrupt world data",
...
async function seedDefaultPlugins(userId, serverId, projectIds, mcVersion) {
  ...
  const ids = (Array.isArray(projectIds) ? projectIds : [projectIds]).filter(
    Boolean,
  );
  for (const pid of ids) {
    try {
      ...
      if (hit) {
        ...
        audit(userId, "plugin.seed", serverId, null, {
          name: hit.filename,
          project: pid,
          cached: true,
          mc: hit.mc,
      ...
      if (!dl.ok) {
        console.warn(`[seed] download failed ${pid}: ${dl.status}`);
        continue;
      }
      ...
      try {
        await pluginCache.writeIntoCache(
          pid,
          meta.filename,
          buf,
          {
        ...
        console.warn(`[plugin-cache] writeIntoCache ${pid}:`, err.message);
      }
      console.log(
        `[plugin-cache] MISS ${pid}@${mcVersion || "any"} → fetched + cached`,
      );
      audit(userId, "plugin.seed", serverId, null, {
        name: safe,
        project: pid,
...
router.post("/:id/start", async (req, res) => {
  ...
  try {
    ...
    if (pubtun.isAvailable()) {
      pubtun
        .start(s.id, internalListenPort(s))
    ...
    if (s.playit_secret && playit.isAvailable()) {
      playit
        .start(s.id, bedrockLocalPort(s), s.playit_secret)
...
router.delete("/:id", async (req, res) => {
  ...
  {
    const sec = getSharedPlayitSecret();
    if (sec) playit.deleteServerTunnels(sec, s.id).catch(() => {});
  }
  await dc.removeServer(s);
  if (s.proxy_id && railway.isConfigured()) {
    try {
      await railway.deleteTcpProxy(s.proxy_id);
    } catch (err) {
...
router.post("/:id/promote", async (req, res) => {
  ...
  if (!s) return;
  const prev = db
    .prepare("SELECT id, container_id FROM servers WHERE is_public = 1")
    .get();
  db.prepare("UPDATE servers SET is_public = 0").run();
  db.prepare("UPDATE servers SET is_public = 1 WHERE id = ?").run(s.id);
  audit(req.user.id, "server.promote", s.id, req.ip, {
    previous_public: prev?.id,
  });
...
router.patch("/:id", async (req, res) => {
  ...
  if (
    ...
    !["peaceful", "easy", "normal", "hard"].includes(updates.difficulty)
  )
    return res.status(400).json({ error: "Invalid difficulty" });
  if (
    updates.gamemode != null &&
    !["survival", "creative", "adventure", "spectator"].includes(
      updates.gamemode,
    )
  )
  ...
  if (updates.scheduled_restart_at !== undefined) {
    ...
    if (v === "" || v === null) updates.scheduled_restart_at = null;
      ...
      return res.status(400).json({
        error:
          "scheduled_restart_at must be HH:MM (24h UTC) or empty to disable",
      });
    }
  }
  ...
  for (const k of dbCols) {
    if (updates[k] !== undefined) {
      dbSets.push(`${k} = ?`);
      dbVals.push(updates[k]);
    }
  }
  if (dbSets.length) {
    dbVals.push(s.id);
    db.prepare(`UPDATE servers SET ${dbSets.join(", ")} WHERE id = ?`).run(
      ...dbVals,
    );
  }
  ...
  if (updates.max_players !== undefined)
    props["max-players"] = String(merged.max_players);
  ...
  if (updates.hardcore !== undefined)
    props["hardcore"] = updates.hardcore ? "true" : "false";
  if (updates.view_distance !== undefined) {
    const v = Math.max(
      3,
      Math.min(32, parseInt(updates.view_distance, 10) || 8),
    );
  ...
  if (
    req.body?.restart &&
    s.container_id &&
    !String(s.container_id).startsWith("stub-")
  ) {
    try {
...
router.get("/:id/progress", (req, res) => {
  ...
  try {
    if (fs.existsSync(jarPath))
      jarSizeMb = Math.round(fs.statSync(jarPath).size / (1024 * 1024));
  } catch {}
  const jarReady = jarSizeMb > 0;
  const worldReady =
    /Preparing level|Preparing start region|Loaded \d+ recipes/.test(logs);
  ...
  const phases = [
    { id: "port", label: "Reserve internal port", done: !!s.port },
    {
      id: "jar",
      label: jarReady
        ? `Download server JAR (${jarSizeMb} MB)`
        : "Download server JAR",
      done: jarReady,
    },
...
router.get("/:id/logs", (req, res) => {
  ...
  try {
    ...
    const state = require("../lib/jvm-controller").__getState(id);
    res.json({
      logs: state?.logs ? state.logs.slice(-n) : [],
      note: state ? null : "no live process state (server may not be running)",
    });
  } catch (err) {
...
router.patch("/:id/webhook", (req, res) => {
  ...
  if (
    url &&
    !/^https:\/\/(discord(app)?\.com|ptb\.discord\.com)\/api\/webhooks\//i.test(
      url,
    )
  ) {
    return res.status(400).json({
      error:
        "Must be a https://discord.com/api/webhooks/... URL or empty to clear",
    });
  }
  db.prepare("UPDATE servers SET discord_webhook = ? WHERE id = ?").run(
    url || null,
    s.id,
  );
...
router.get("/:id/diag", async (req, res) => {
  ...
  function probe(host, port, timeout = 4000) {
    return new Promise((resolve) => {
      ...
      sock.on("error", (err) =>
        resolve({ ok: false, err: err.message, code: err.code }),
      );
      sock.on("timeout", () => {
        sock.destroy();
  ...
  const remoteCheck = s.tunnel_port
    ? await probe(s.tunnel_host, s.tunnel_port, 6000)
    : null;
  let dnsLookup = null;
  try {
    dnsLookup = await dns.lookup(s.tunnel_host || "bore.pub", { all: true });
  } catch (e) {
    dnsLookup = { err: e.message };
  }
...
router.post("/:id/playit/auto-enable", (req, res) => {
  ...
  if (!sharedSecret) {
    return res.status(503).json({
      error:
        "no shared playit secret yet — fall back to claim flow (the first owner claim captures it)",
    });
  }
  ...
  // Per-server mode: each server gets its OWN Bedrock tunnel, so multiple can
  // hold Bedrock at once — no takeover needed. Legacy shared mode keeps the
  // one-at-a-time lock below.
  const perServerBedrock = process.env.BEDROCK_PER_SERVER === "1";
  const takeover = perServerBedrock || (req.body && req.body.takeover === true);
  const others = perServerBedrock
    ? []
    : db
        .prepare(
  ...
  if (others.length && !takeover) {
    const active =
      others.find((o) =>
        ["online", "running", "starting"].includes(o.status),
      ) || others[0];
    return res.status(409).json({
      ...
      conflict: {
        server_id: active.id,
        name: active.name,
        online: ["online", "running", "starting"].includes(active.status),
      },
    });
  }
  try {
    if (takeover && others.length) {
      ...
      audit(req.user.id, "server.playit_takeover", s.id, req.ip, {
        from: others.map((o) => o.id),
      });
    }
    ...
    if (
      ...
      playit
        .start(s.id, bedrockLocalPort(s), sharedSecret)
        .catch((err) =>
...
router.post("/:id/playit/claim/start", (req, res) => {
  ...
  if (!playit.isAvailable())
    return res.status(503).json({ error: "playit agent not installed" });
  try {
    const out = playit.claimStart(
      s.id,
      `CraftHost-${s.name || s.id.slice(0, 8)}`,
    );
  ...
  try {
    ...
    if (out.status === "connected" && playit.isAvailable()) {
      ...
      if (
        ...
        playit
          .start(s.id, bedrockLocalPort(s), fresh.playit_secret)
          .catch((err) =>
            console.warn("[playit claim] hot-start:", err.message),
          );
      }
      ...
      if (
        fresh?.playit_secret &&
        isUnlimitedUser(req.user) &&
        !getSharedPlayitSecret()
      ) {
...
router.post("/:id/playit", async (req, res) => {
  ...
  try {
    db.prepare(
      "UPDATE servers SET playit_secret = ?, playit_host = NULL, playit_port = NULL WHERE id = ?",
    ...
    res
      .status(500)
      .json({ error: err.message || "failed to update playit secret" });
  }
});
...
router.post("/:id/swap-jar", async (req, res) => {
  ...
  try {
    ...
    for (const f of STALE_CONFIGS) {
      try {
        await fsp.rm(path.join(serverRoot, f), {
          recursive: true,
          force: true,
        });
      } catch {}
    ...
    if (PAPER_FAMILY.has(s.type) && type === "vanilla") {
      ...
      try {
        await fsp.writeFile(
          path.join(serverRoot, ".crafthost-vanilla-safemode"),
          String(Date.now()),
        );
      } catch {}
      try {
        await fsp.rm(path.join(serverRoot, "world", "datapacks"), {
          recursive: true,
          force: true,
        });
      } catch {}
      try {
        await fsp.rm(path.join(serverRoot, "world", "data"), {
          recursive: true,
          force: true,
        });
      } catch {}
      try {
        await fsp.rm(path.join(serverRoot, "world", "level.dat"), {
          recursive: true,
          force: true,
        });
      } catch {}
      try {
        await fsp.rm(path.join(serverRoot, "world", "level.dat_old"), {
          recursive: true,
          force: true,
        });
      } catch {}
  ...
  async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        console.warn("[import-world] stop failed:", err.message);
      }
      ...
      if (!isWrappedLayout) {
        const levelDatEntry = entries.find((e) =>
          /(^|\/)level\.dat$/.test(e.entryName),
        );
        if (levelDatEntry) {
          internalsRootPrefix = path.posix
            .dirname("/" + levelDatEntry.entryName)
            .replace(/^\//, "");
          // Empty prefix means level.dat is at the very root.
        } else {
          return res.status(400).json({
            error:
              "Zip does not look like a Minecraft world (no level.dat found; expected world/level.dat or level.dat at root)",
          });
        }
      }
      ...
      if (isWrappedLayout) {
        ...
        const prefix = internalsRootPrefix
          ? internalsRootPrefix.replace(/\/+$/, "") + "/"
          : "";
        for (const e of entries) {
          if (e.isDirectory) continue;
          const name = e.entryName.replace(/^\/+/, "");
      ...
      audit(req.user.id, "server.world_import", s.id, req.ip, {
        size: req.file.size,
        restored,
      });
      try {
...
router.post("/:id/icon", iconUpload.single("icon"), async (req, res) => {
  ...
  if (!s) return;
  if (!req.file)
    return res.status(400).json({ error: 'No "icon" file in upload' });
  const fsp = require("fs/promises");
  const path = require("path");
  const DATA_DIR =
    process.env.DATA_DIR || path.resolve(__dirname, "../../data/servers");
...
router.delete("/:id/icon", async (req, res) => {
  ...
  try {
    ...
    if (err.code === "ENOENT")
      return res.json({ ok: true, note: "no icon was set" });
    res.status(500).json({ error: err.message });
  }
});
...
function buildLauncherBat(name, base, token) {
  const BASE = base;
  const TOKEN = token;
  const startUrl = `${BASE}/api/launcher/${TOKEN}/start`;
  const stopUrl = `${BASE}/api/launcher/${TOKEN}/stop`;
  const restartUrl = `${BASE}/api/launcher/${TOKEN}/restart`;
  const statusUrl = `${BASE}/api/launcher/${TOKEN}/status`;
  return [
    "@echo off",
    "setlocal enabledelayedexpansion",
    `title CraftHost - ${name} Launcher`,
    "",
    ...
    "echo.",
    "echo  ╔══════════════════════════════════════════════════╗",
    `echo  ║        CraftHost Launcher — ${name}        ║`,
    "echo  ╚══════════════════════════════════════════════════╝",
    "echo.",
    "echo  Fetching server status...",
    `for /f "usebackq delims=" %%L in (\`curl -s -m 10 "${statusUrl}"\`) do (`,
    "  if not defined _S1 set \"_S1=%%L\" & goto :parse",
    "  :parse",
    "  if not defined _S2 set \"_S2=%%L\" & goto :parse2",
    "  :parse2",
    "  if not defined _S3 set \"_S3=%%L\" & goto :parse3",
    "  :parse3",
    "  if not defined _S4 set \"_S4=%%L\"",
    ")",
    ...
    'echo  [4] Status (re-check)',
    `echo  [5] Open dashboard (${BASE})`,
    'echo  [0] Exit',
    "echo.",
    ...
    'if /i "%STATUS%"=="starting" (echo  Server is already starting! & timeout /t 2 /nobreak >nul & goto :menu)',
    "echo  Starting your server...",
    `for /f "usebackq delims=" %%A in (\`curl -s -m 30 -X POST "${startUrl}"\`) do set "START_REPLY=%%A"`,
    'if not defined START_REPLY set "START_REPLY=ERROR no reply from CraftHost - check your internet"',
    "echo  %START_REPLY%",
    'echo %START_REPLY%| findstr /b /c:"ERROR" >nul && (echo. & pause & goto :menu)',
    "echo  Waiting for server to come online...",
    "set /a TRIES=0",
    ":poll",
    "set /a TRIES+=1",
    `if %TRIES% GTR 60 (echo  Taking longer than usual - check your dashboard: ${BASE} & pause & goto :menu)`,
    "timeout /t 5 /nobreak >nul",
    `for /f "usebackq delims=" %%A in (\`curl -s -m 15 "${statusUrl}"\`) do set "LINE=%%A" & goto :poll_break`,
    ":poll_break",
    'for /f "tokens=1 delims= " %%X in ("%LINE%") do set "ST=%%X"',
    'if /i "%ST%"=="crashed" (echo  Server crashed. Auto-heal will retry. Check dashboard. & pause & goto :menu)',
    'if /i "%ST%"=="online" (',
    "  for /f \"tokens=2 delims= \" %%Y in (\"%LINE%\") do set \"ADDR=%%Y\"",
    "  echo.",
    '  echo  ╔════════════════════════════════════════╗',
    ...
    "",
    ":stop",
    'if /i "%STATUS%"=="offline" (echo  Server is already offline. & timeout /t 2 /nobreak >nul & goto :menu)',
    "echo  Stopping server...",
    `for /f "usebackq delims=" %%A in (\`curl -s -m 30 -X POST "${stopUrl}"\`) do set "STOP_REPLY=%%A"`,
    "echo  %STOP_REPLY%",
    "pause",
    "goto :menu",
    "",
    ":restart",
    'if /i "%STATUS%"=="offline" (echo  Server is offline - start it first. & timeout /t 2 /nobreak >nul & goto :menu)',
    "echo  Restarting server...",
    `for /f "usebackq delims=" %%A in (\`curl -s -m 30 -X POST "${restartUrl}"\`) do set "RESTART_REPLY=%%A"`,
    "echo  %RESTART_REPLY%",
    "pause",
    "goto :menu",
    "",

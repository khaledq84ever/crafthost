...
function buildLauncherBat(name, base, token) {
  ...
  return [
    ...
    "echo  ╚══════════════════════════════════════════════════╝",
    "echo.",
    `curl -s -m 10 -o "%STATUS_FILE%" "${statusUrl}"`,
    ...
    'if not defined START_REPLY set "START_REPLY=ERROR no reply from CraftHost"',
    "echo  %START_REPLY%",
    ...
    `curl -s -m 15 -o "%STATUS_FILE%" "${statusUrl}"`,
    '<"%STATUS_FILE%" set /p _S1=',
    'if not defined _S1 (set "ST=error" & goto :poll_done)',
    'set "ST=%_S1%"',
    'set "_S1="',
    ':poll_done',
    'del "%STATUS_FILE%" 2>nul',
    'if /i "%ST%"=="crashed" (echo  Server crashed. Auto-heal will retry. & pause & goto :menu)',
    'if /i "%ST%"=="online" (',
    `  curl -s -m 10 -o "%STATUS_FILE%" "${statusUrl}"`,
    '  <"%STATUS_FILE%" set /p _S1=',
    '  <"%STATUS_FILE%" set /p _S2= & set /p _S2=',

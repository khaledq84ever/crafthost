...
const EDITABLE_EXT = new Set([
  ...
  '.log', '.md', '.sh', '.bat', '.mcmeta', '.snbt', '.csv', '.xml', '.html', '.css', '.js'
]);
...
function safeJoin(root, userPath) {
  const cleaned = String(userPath || '').replace(/\\/g, '/').replace(/\0/g, '');
...
router.get('/', async (req, res) => {
  ...
  try {
    ...
    if (!stat || !stat.isDirectory()) return res.status(404).json({ error: 'Not a directory' });
    ...
    res.status(err.code === 'EPATH' ? 400 : 500).json({ error: err.code === 'EPATH' ? 'Invalid path' : 'List failed' });
  }
});
...
router.get('/read', async (req, res) => {
  ...
  try {
    ...
    if (st.size > MAX_EDITABLE_BYTES) return res.status(413).json({ error: 'File too large to edit' });
...
router.put('/write', async (req, res) => {
  ...
  try {
    ...
    if (content.length > MAX_EDITABLE_BYTES) return res.status(413).json({ error: 'Too large' });
    const root = serverRoot(s.id);
    ...
    res.status(err.code === 'EPATH' ? 400 : 500).json({ error: err.code === 'EPATH' ? 'Invalid path' : 'Write failed' });
  }
});
...
router.post('/mkdir', async (req, res) => {
  ...
  try {
    ...
    res.status(err.code === 'EPATH' ? 400 : 500).json({ error: err.code === 'EPATH' ? 'Invalid path' : 'Mkdir failed' });
  }
});
...
router.post('/rename', async (req, res) => {
  ...
  try {
    ...
    audit(req.user.id, 'file.rename', s.id, req.ip, { from: req.body?.from, to: req.body?.to });
    res.json({ ok: true });
  } catch (err) {
    res.status(err.code === 'EPATH' ? 400 : 500).json({ error: err.code === 'EPATH' ? 'Invalid path' : 'Rename failed' });
  }
});
...
router.delete('/', async (req, res) => {
  ...
  try {
    ...
    res.status(err.code === 'EPATH' ? 400 : 500).json({ error: err.code === 'EPATH' ? 'Invalid path' : 'Delete failed' });
  }
});
...
router.get('/download', async (req, res) => {
  ...
  try {
    ...
    res.status(err.code === 'EPATH' ? 400 : 500).json({ error: err.code === 'EPATH' ? 'Invalid path' : 'Download failed' });
  }
});
...
const upload = multer({
  storage: multer.diskStorage({
    destination: (req, file, cb) => {
      try {
        const s = db.prepare('SELECT id, user_id FROM servers WHERE id = ?').get(req.params.id);
        if (!s || s.user_id !== req.user.id) return cb(new Error('Not authorized'));
        const root = serverRoot(s.id);
...
function audit(user_id, action, resource_id, ip, metadata) {
  try {
    db.prepare('INSERT INTO audit_log (user_id, action, resource_type, resource_id, ip, metadata) VALUES (?, ?, ?, ?, ?, ?)')
      .run(user_id, action, 'file', resource_id, ip || null, metadata ? JSON.stringify(metadata) : null);
  } catch {}
}

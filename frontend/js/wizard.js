...
const SERVER_TYPES = [
  { id: 'paper',    name: 'Paper',    c: '#F57C00', desc: 'High-performance fork. Plugins supported.' },
  { id: 'vanilla',  name: 'Vanilla',  c: '#7CB342', desc: 'Pure Mojang. No mods, no plugins.' },
  { id: 'purpur',   name: 'Purpur',   c: '#283593', desc: 'Highly configurable Paper fork.' },
  { id: 'fabric',   name: 'Fabric',   c: '#01579B', desc: 'Lightweight modding API for mods.' },
  { id: 'neoforge', name: 'NeoForge', c: '#FF6F00', desc: 'Modern Forge fork — for heavy modpacks.' },
  { id: 'custom',   name: 'Custom JAR', c: '#FFB300', desc: 'Upload your own .jar file (up to 500MB).' },
];
...
const PLANS = [
  { id: 'free', name: 'Free', price: 0, ram: 2048, cpu: 2, slots: 50, badge: 'FREE' },
];
...
function renderWizard() {
  ...
  `;
  m.onclick = (e) => { if (e.target === m) closeWizard(); };
}
...
function renderStep2() {
  if (wizState.type === 'custom') {
    return `
      ...
      ${wizState.customJar ? `<div class="card mt-4" style="display:flex;justify-content:space-between;align-items:center;">
  ...
  if (!cached) {
    // Lazy fetch
    fetch(`/api/versions/${encodeURIComponent(wizState.type)}?limit=50`, { credentials: 'include' })
      .then(r => r.ok ? r.json() : { versions: [] })
      .then(d => {
        wizState._liveVersions[wizState.type] = d.versions || [];
        if (wizState._liveVersions[wizState.type].length) {
          wizState.version = safeVersionForPlan(wizState.plan, wizState._liveVersions[wizState.type]);
        }
        renderWizard();
      })
      .catch(() => { wizState._liveVersions[wizState.type] = []; renderWizard(); });
    return `
  ...
  const planRisky = wizState.plan === 'free' && isHeavyVersion(wizState.version);
        ...
        ${groupKeys.map(gk => `
          <optgroup label="Minecraft ${gk}">
            ${groups[gk].map(v => {
              ...
              if (wizState.plan === 'free' && isHeavyVersion(id)) tags.push('⚠ may OOM');
              if (id === recommended && wizState.plan === 'free') tags.push('✓ recommended');
...
function handleJar(e) {
  ...
  if (!f) return;
  if (f.size > 500 * 1024 * 1024) { toast('File too large (max 500MB)', 'error'); return; }
  if (!f.name.endsWith('.jar')) { toast('Only .jar files accepted', 'error'); return; }
...
async function wizDeploy() {
  if (!wizState.name || !wizState.type || !wizState.plan) {
    toast('Missing required fields', 'error'); return;
  }
  ...
  try {
    ...
    if (wizState.customJar?.file) {
      ...
      const up = await fetch('/api/jars', { method: 'POST', body: fd, credentials: 'include' });
      if (!up.ok) {
    ...
    if (btn) { btn.disabled = false; btn.textContent = '🚀 Deploy server'; }
  }
}
...
async function pollDeployProgress(r) {
  ...
  while (true) {
    let data = null;
    try { data = await api(`/api/servers/${sid}/progress`); } catch {}
    if (data) {
      ...
      if (cur && lastPhaseId === cur.id && hintEl) {
        if (cur.id === 'jar') hintEl.textContent = 'Downloading from papermc.io — first launch only, future starts are instant.';
        else if (cur.id === 'world') hintEl.textContent = 'Generating chunks for the spawn region…';
        else if (cur.id === 'jvm') hintEl.textContent = 'Loading mods, libraries, and recipes…';
        else if (cur.id === 'tunnel') hintEl.textContent = 'Allocating a real public IP via bore.pub…';
      } else if (cur) {
        lastPhaseId = cur.id;
      ...
      if (data.oom) {
        if (hintEl) hintEl.textContent = '🔧 First-boot OOM — auto-healing to Paper 1.20.1 (one moment)…';
      ...
      if (!data.oom && data.exit_code !== null && data.exit_code !== 0) {
        renderDeployFailure(sid, `Server crashed on first boot (exit code ${data.exit_code}). Check console logs for details.`);
        return;
      }
      ...
      if (data.ready) {
        ...
        try {
          ...
          showDeploySuccess({
            id: sid,
            port: me.port,
            proxy: me.tunnel_host ? { host: me.tunnel_host, port: me.tunnel_port } : null,
            is_public: !!me.is_public,
            auto_started: true,
          }, h.public_mc_host || '', h.public_mc_port || 25565);
        } catch {
    ...
    if (Date.now() - startTs > 180_000) {
      renderDeployFailure(sid, 'Boot is taking longer than expected. The server may still come online — check the dashboard.');
      return;
    }
...
function showDeploySuccess(r, publicHost, publicPort) {
  ...
  const address = hasTunnel
    ...
    : isPublic ? `${publicHost}:${publicPort}` : `(waiting for tunnel — refresh in 15s)`;
